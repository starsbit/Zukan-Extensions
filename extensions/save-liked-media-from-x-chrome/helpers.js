...
export function summarizeBatchResults(payload) {
...
export function describeCobaltError(payload) {
  ...
  if (payload?.status === 'local-processing') {
    return 'Cobalt returned a local-processing response that this extension cannot use.';
  }
  return 'Cobalt returned an unexpected response.';
}
